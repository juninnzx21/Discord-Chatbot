...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    db.get('SELECT * FROM pontos WHERE usuario_id = ?', [idUsuario], async (err, row) => {
      if (err) {
        console.error(err);
        return interaction.reply({ content: '<1198037616956821515> | Ocorreu um erro ao buscar os dados.', ephemeral: true });
      }

      if (!row) {
        return interaction.reply({ content: '<1198037616956821515> | Você ainda não patrulhou após o último reset..', ephemeral: true });
      }
      ...
      if (tempoAbertoAtual == 0) {
        ...
        interaction.reply({ content: ` | Você está com ponto aberto há: ${tempoAbertoAtualFormatado}`, ephemeral: true });
      }
    });
  },
};
