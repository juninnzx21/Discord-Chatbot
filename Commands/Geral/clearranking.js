...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (!interaction.member.permissions.has("MANAGE_MESSAGES")) {
      ...
      interaction.reply({
        content:
          "<1197986783808471171> Você tem certeza de que deseja limpar completamente o banco de dados de pontos?",
        components: [row],
        ephemeral: true,
      });
      ...
      collector.on("collect", async (i) => {
        if (i.customId === "confirm_clear") {
          ...
          db.run("DELETE FROM pontos", (err) => {
            if (err) {
              ...
              return interaction.editReply({
                content: "<1198037616956821515> Ocorreu um erro ao limpar o banco de dados.",
                components: [],
              });
            }
          ...
          interaction.editReply({
            content: "<1198037616956821515> Limpeza do banco de dados cancelada.",
            components: [],
          });
        }
    ...
    collector.on("end", (collected) => {
      if (collected.size === 0) {
        interaction.followUp({
          content: "<1198037616956821515> Tempo expirado. Ação cancelada.",
          ephemeral: true,
        });
      }
    });
