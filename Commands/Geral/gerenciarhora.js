...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (!interaction.member.permissions.has("MANAGE_MESSAGES")) {
            interaction.reply({
              content: `<1198037616956821515> | Você não possui permissão para utilizar este comando.`,
              ephemeral: true,
            });
        }
...
function adicionarHoras(usuarioId, tempo, interaction, canalLog) {
    ...
    db.get('SELECT * FROM pontos WHERE usuario_id = ?', [usuarioId], async (err, row) => {
        ...
        if (!row) {
            await interaction.reply({ content: '> <1198037616956821515> | Usuário não encontrado no banco de dados.', ephemeral: true });
            return;
        }
        ...
        await interaction.reply({ content: ` | Horas adicionadas com sucesso para o usuário com <@${usuarioId}>.`, ephemeral: true });
...
function removerHoras(usuarioId, tempo, interaction, canalLog) {
    db.get('SELECT * FROM pontos WHERE usuario_id = ?', [usuarioId], async (err, row) => {
        ...
        if (novosIntervalos.length === 0) {
            await interaction.reply({ content: '> <1198037616956821515> | O usuário não tem horas registradas.', ephemeral: true });
            return;
        }

        const totalHoras = novosIntervalos.reduce((acc, intervalo) => acc + intervalo, 0);
        if (totalHoras < tempo) {
            await interaction.reply({ content: '> <1198037616956821515> | O usuário não tem horas suficientes para remover.', ephemeral: true });
            return;
        }
        ...
        await interaction.reply({ content: ` | Horas removidas com sucesso do usuario usuário <@${usuarioId}>.`, ephemeral: true });
