...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (!interaction.member.permissions.has("MANAGE_MESSAGES")) {
      return interaction.reply({ content: `<1198037616956821515> | Você não possui permissão para utilizar este comando.`, ephemeral: true });
    }
    ...
    client.on('interactionCreate', async (interaction) => {
      ...
      if (interaction.customId === 'remover_ausencia_dropdown') {
        ...
        db.run('DELETE FROM ausencia WHERE usuario_id = ?', [idUsuario], (err) => {
          if (err) {
            console.error(err);
            return interaction.reply({ content: `> <1198037616956821515> | Houve um erro ao remover o usuário da lista de ausência.`, ephemeral: true });
          }
          const canalLog = client.channels.cache.get(canalLogId);
          interaction.reply({ content: `> | Usuário removido com sucesso da lista de ausência.`, ephemeral: true });
          if (canalLog) {
