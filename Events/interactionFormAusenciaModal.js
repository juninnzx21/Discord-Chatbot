...
module.exports = async (client, interaction) => {
    ...
    if (interaction.isModalSubmit() && interaction.customId === "modal_ausencia") {
        ...
        db.get('SELECT * FROM ausencia WHERE usuario_id = ?', [interaction.user.id], (err, row) => {
            ...
            if (row) {
                return interaction.reply({ content: `>  | ${interaction.user} Você já possui uma ausência registrada.`, ephemeral: true });
            }
        });
        ...
        if (dataAusencia < dataAtual) {
            return interaction.reply({ content: `>  | ${interaction.user} A data de ausência não pode ser anterior à data atual. Por favor, insira uma data válida.`, ephemeral: true });
        }
        if (dataAusencia === dataAtual) {
            return interaction.reply({ content: `>  | ${interaction.user} A data de ausência não pode ser igual à data atual. Por favor, insira uma data válida acima de hoje.`, ephemeral: true });
        }
        ...
        db.run('INSERT INTO ausencia (usuario_id, tempo_off) VALUES (?, ?)', [interaction.user.id, tempoOff], (err) => {
            if (err) {
                console.error(err);
                return interaction.reply({ content: `>  | ${interaction.user} Houve um erro ao registrar sua ausência.`, ephemeral: true });
            }
    
            interaction.reply({ content: `>  | ${interaction.user} Sua ausência foi registrada até ${tempoOff}, dentro desse prazo fica impedido de bater-ponto.`, ephemeral: true });
